...
router.get('/excel/template', requireRole('admin', 'chef'), async (req, res) => {
  try {
    ...
    applyStyle(ws.getCell('A2'), {
      value: 'Remplissez les infos chantier (lignes 4-9), puis les équipements à partir de la ligne 13. Ne pas modifier les en-têtes. NIC 2 = 2ème carte réseau (Dante, AV LAN…)',
    ...
    // ── Bloc infos chantier (lignes 4-9) ─────────────────────────
    ...
    infoRows.forEach(([rowNum, leftLabel, rightLabelCell, rightLabel]) => {
      ...
      // Valeur gauche B:E merged (input)
