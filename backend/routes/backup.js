...
function pruneAutoBackups() {
  ...
  const autoFiles = fs.readdirSync(BACKUP_DIR)
    .filter(f => f.endsWith('.json') && f.startsWith('backup_') && !f.includes('manual'))
  ...
  autoFiles.slice(MAX_AUTO_BACKUPS).forEach(f => {
    try {
      ...
      console.log(`[Backup] Ancienne sauvegarde supprimée : ${f.name}`);
    } catch (e) {
      console.error(`[Backup] Erreur suppression ${f.name} :`, e.message);
    }
  });
}
