...
export default function Chantiers() {
  ...
  const createChantier = async () => {
    ...
    setSaving(true)
    try {
      const res = await axios.post('/api/chantiers', {
        ...form,
        salles: formSalles.filter(s => s.trim())
      })
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 16, padding: 24, marginBottom: 24, borderTop: '3px solid #10B981', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {formSalles.map((nom, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <input
                      value={nom}
                      onChange={e => setFormSalles(s => s.map((v, j) => j === i ? e.target.value : v))}
                      placeholder={`Ex: Salle de réunion ${i + 1}`}
                      style={{ ...inputStyle, flex: 1 }}
                    />
                    <button onClick={() => setFormSalles(s => s.filter((_, j) => j !== i))}
                      style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444', borderRadius: 8, padding: '8px 10px', cursor: 'pointer', fontSize: 15, flexShrink: 0 }}>
                      ✕
                    </button>
                  </div>
                ))}
                {formSalles.length === 0 && (
                  <div style={{ fontSize: 12, color: '#4B5563', fontStyle: 'italic' }}>
                    Aucune salle — vous pourrez en ajouter depuis la fiche chantier
                  </div>
                )}
              </div>
            </div>
                ...
                <div key={c.id}
                  ...
                  <div style={{ padding: '16px 18px 14px' }} onClick={() => navigate('/chantiers/' + c.id)}>
                    ...
                    <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 99, overflow: 'hidden', marginBottom: 12 }}>
                      <div style={{ height: '100%', width: pct + '%', background: statusColor, borderRadius: 99, opacity: 0.9 }} />
                    </div>
                  </div>
