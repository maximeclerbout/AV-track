...
export default function Utilisateurs() {
  ...
  const addCategorie = async () => {
    ...
    try {
      ...
      refreshCats(); setNewCat(''); setNewCatCouleur('#00D4FF')
  ...
  const tabs = [
    { key: 'utilisateurs', label: 'Utilisateurs', icon: icons.user },
    { key: 'categories', label: "Types d'équipements", icon: icons.tag },
    ...(currentUser?.role === 'admin' ? [{ key: 'email', label: 'Notifications email', icon: icons.mail }] : []),
  ]
    ...
    <Layout chantiers={chantiers}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: 26, fontWeight: 800, marginBottom: 4 }}>Paramètres</h1>
          <p style={{ color: '#6B7280', fontSize: 14 }}>Gestion des utilisateurs, types d'équipements et notifications</p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid rgba(255,255,255,0.08)', marginBottom: 28, overflowX: 'auto' }}>
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={TAB_STYLE(tab === t.key)}>
              <Icon d={t.icon} size={14} color={tab === t.key ? '#00D4FF' : '#7b8096'} />
              {t.label}
            </button>
          ))}
        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                  ...
                  <div>
                    ...
                    <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                      placeholder="marc.dupont@entreprise.fr" style={inputStyle} />
                  </div>
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
                          ...
                          {!u.actif && (
                            <span style={{ fontSize: 11, color: '#6B7280', background: 'rgba(107,114,128,0.1)', padding: '2px 8px', borderRadius: 20 }}>Desactive</span>
                          )}
                          {u.id === currentUser?.id && (
                            <span style={{ fontSize: 11, color: '#10B981', background: 'rgba(16,185,129,0.1)', padding: '2px 8px', borderRadius: 20 }}>Vous</span>
                          )}
                        </div>
        ...
        {tab === 'email' && currentUser?.role === 'admin' && (
          <div style={{ background: '#13151E', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 16, padding: 24 }}>
            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 700, marginBottom: 16, color: '#10B981' }}>Configuration SMTP</div>
